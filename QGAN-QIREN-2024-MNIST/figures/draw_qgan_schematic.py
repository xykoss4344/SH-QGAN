...
ax.set_xlim(0, 105) # Expanded slightly
ax.set_ylim(-10, 100)
ax.axis('off')
...
# Sequence: W^1, S(h), W^2, S(h), ..., W^{L-1}, S(h), W^L
...
# Wiring BN to QC
# Draw common bus line ABOVE the circuit
...
# Drop arrows directly to top of every S(h) block
for i in [1, 3, 6]: # Indices of S(h) in the sequence
    sh_x = x_pos[i] + 1.75 # Center of the S(h) box
...
for idx, y in enumerate([93, 88.5, 84]):
    if idx == 2:
        ax.text(15, y, f'$|\\phi_{{n}}>$', ha='center', va='center', fontsize=14)
    else:
        ax.text(15, y, f'$|\\phi_{idx+1}>$', ha='center', va='center', fontsize=14)
...
for idx, y in enumerate([93, 88.5, 84]):
    if idx == 2:
        ax.text(58, y, f'$|\\phi_{{n}}>$', ha='center', va='center', fontsize=14)
    else:
        ax.text(58, y, f'$|\\phi_{idx+1}>$', ha='center', va='center', fontsize=14)
    num = 'n' if idx == 2 else str(idx+1)
...
draw_arrow(88, 55, 89, 62) # Up to Atom
draw_arrow(88, 55, 89, 48) # Down to Cell
...
draw_arrow(40, 28, 50, 16)
draw_arrow(75, 28, 60, 16)

# ── 6. Classical Critic ──
crit_bg = FancyBboxPatch((40, -2), 30, 18, boxstyle='round,pad=0.2,rounding_size=1', fc='#f3e5f5', ec='black', zorder=1)
ax.add_patch(crit_bg)
ax.text(55, -4, 'Classical Critic', ha='center', va='center', fontsize=18, fontweight='bold', bbox=dict(facecolor='#ffca28', edgecolor='black'))

y_1 = [12, 10, 8, 6, 4]; y_2 = [11, 9, 7, 5]; y_3 = [10, 8, 6]; y_4 = [8]
x_n = [45, 52, 59, 65]
for n1 in y_1:
...
draw_box(15, 5, 20, 6, 'Wasserstein distance', fc='#ffcc80', fs=14)

ax.plot([10, 15], [8, 8], 'k--', lw=2.5)
ax.plot([10, 10], [8, 77], 'k--', lw=2.5)
draw_arrow(10, 77, 45, 77, ls='--', lw=2.5) # Top loop over generator back into QC
ax.text(15, 79, 'Updates', fontsize=14, fontweight='bold')

ax.plot([25, 25], [5, -8], 'k--', lw=2.5)
ax.plot([25, 85], [-8, -8], 'k--', lw=2.5)
ax.plot([85, 85], [-8, 8], 'k--', lw=2.5)
draw_arrow(85, 8, 70, 8, ls='--', lw=2.5)
ax.text(55, -10, 'Updates', fontsize=14, fontweight='bold', ha='center')

plt.savefig(os.path.join(OUT_DIR, 'architecture_qgan_styled.png'), dpi=300, bbox_inches='tight')
print("Successfully rendered styled QGAN architecture diagram.")
