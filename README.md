...
| `train_crystal_classical_ablation.py` | classical-generator ablation (`models/Classical_Crystal.py`) |
| `baselines/` | element-substitution baseline |
| `dft/` | QE/VASP inputs and DFT validation scripts |
| `experiments/` | archived results from previous waves (git-ignored) |

Old v4-v7 eval scripts, SSIM plots and MNIST-era code were removed; they live at
the `pre-cleanup` git tag.
